  ...
  return (
    <div ref={containerRef} class="relative">
      ...
      <button
        ...
        class="flex items-center gap-1 px-1.5 py-0.5 text-base border border-border text-text-muted hover:border-accent hover:text-accent transition-colors disabled:opacity-50 disabled:cursor-not-allowed max-w-[140px]"
        ...
        <div class="absolute top-full left-0 mt-1 w-72 bg-bg-surface border border-border shadow-lg z-50">
          ...
          <div class="p-2 border-b border-border">
            <input
              ...
              class="w-full px-2 py-1 text-base bg-bg border border-border text-text placeholder:text-text-faint focus:border-accent"
            />
          </div>
          ...
          <div class="max-h-64 overflow-y-auto">
            <Show when={isLoading()}>
              <div class="px-3 py-2 text-base text-text-faint">Searching...</div>
            </Show>
            
            <Show when={!isLoading() && searchResults().length === 0}>
              <div class="px-3 py-2 text-base text-text-faint">No models found</div>
            </Show>
              ...
              {(model) => {
                ...
                return (
                  <button
                    ...
                    class="w-full px-3 py-1.5 text-left text-base hover:bg-bg-elevated transition-colors flex flex-col gap-0.5"
                    classList={{ "bg-accent/10": isSelected() }}
                  >
                    <span class="text-text font-medium truncate">{model.modelId}</span>
                    <span class="text-text-faint text-base">{model.providerId}</span>
                  </button>
                );
              }}
            </For>
